...
@section('breadcrumbs')
    {!! Breadcrumbs::render('admin_services') !!}
@endsection
@section('header')
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <a href="{{ route('admin.services.create') }}" class="btn btn-primary btn-flat" target="_blank">
    ...
    <table class="table table-hover">
        ...
        @foreach($list as $item)
            <tr>
                ...
                <td>{{$item->name}}</td>
                <td>
                    <a href="{{route('admin.services.edit',['id'=>$item->id])}}" class="btn btn-default"
                       data-toggle="tooltip" data-placement="top" target="_blank" title="Editar #{{ $item->id }}"
                    >
                        <i class="fa fa-pencil"></i>
                    </a>
                    <a href="{{route('admin.services.show',['id'=>$item->id])}}" class="btn btn-default"
                       data-toggle="tooltip" data-placement="top" target="_blank" title="Visualizar #{{ $item->id }}"
                    >
                        <i class="fa fa-search"></i>
                    </a>
                    <a href="{{route('admin.services.print',['id'=>$item->id])}}" class="btn btn-default"
